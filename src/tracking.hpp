...
class TrackingData {
    CircularBuffer<std::vector<String>> dummy; // TODO: Figure out why I cannot remove this without the whole thing segfaulting
    CircularBuffer<std::vector<String>> BSSIDBuffer;
    CircularBuffer<std::vector<int32_t>> RSSIBuffer;

public:
    TrackingData(int bufSize) : dummy(bufSize), BSSIDBuffer(bufSize), RSSIBuffer(bufSize) {}
